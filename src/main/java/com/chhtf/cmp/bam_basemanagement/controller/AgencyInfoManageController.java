...
public class AgencyInfoManageController {
	...
	public @ResponseBody ResponseEntity findAll(){
		...
		List<AgencyInfo> list = service.list();
		data.put("list", list);
	...
	public @ResponseBody ResponseEntity findByPage(@RequestBody RequestEntity requestEntity){
		...
		List<AgencyInfo> list = service.findListByCondition(requestEntity.getAgencyInfo());
		List<AgencyInfoExtends> extendsList = service.packExtendsListFrom(list);
	...
	public @ResponseBody ResponseEntity findById(@RequestBody RequestEntity requestEntity){
		...
		AgencyInfo record = service.get(requestEntity.getId());
	...
	public @ResponseBody ResponseEntity add(@RequestBody RequestEntity requestEntity){
		ResponseEntity responseEntity = new ResponseEntity();
		try {
			AgencyInfo agencyInfo = requestEntity.getAgencyInfo();
			if(agencyInfo.getUserId()==null||agencyInfo.getUserId()==0) {
				UserInfo userInfo = new UserInfo();
				userInfo.setPhone(agencyInfo.getPhone());
				userInfoservice.add(userInfo);
				agencyInfo.setUserId(userInfo.getId());
			}
			service.add(agencyInfo);
			Map<String, Object> data = new HashMap<String, Object>();
			
			data.put("record", requestEntity.getAgencyInfo());
			responseEntity.setData(data);
		} catch (Exception e) {
	...
	public @ResponseBody ResponseEntity update(@RequestBody RequestEntity requestEntity){
		ResponseEntity responseEntity = new ResponseEntity();
		try {
			service.update(requestEntity.getAgencyInfo());
		} catch (Exception e) {
			responseEntity.setSuccess(CommonConstant.RESPONSE_FAIL);
			responseEntity.setErrorMsg(e.getMessage());
		}
