...
public class TestApplicationManageController {
	
	@Autowired
	TestApplicationService service;
	...
	public @ResponseBody ResponseEntity findAll(){
		...
		List<TestApplication> list = service.list();
		data.put("list", list);
	...
	public @ResponseBody ResponseEntity findByPage(@RequestBody RequestEntity requestEntity){
		...
		List<TestApplication> list = service.findListByCondition(requestEntity.getTestApplication());
		List<TestApplicationExtends> extendsList = service.packExtendsListBy(list);
	...
	public @ResponseBody ResponseEntity findById(@RequestBody RequestEntity requestEntity){
		...
		TestApplication record = service.get(requestEntity.getId());
		TestApplicationExtends extendsRecord = service.packExtendsBy(record);
		data.put("record", extendsRecord);
		responseEntity.setData(data);
		return responseEntity;
	}
	...
	/**
	 *	处理
	 * @param dealwithTestApplication
	 * @return
	 */
	@RequestMapping("dealwithTestApplication")
	public @ResponseBody ResponseEntity dealwithTestApplication(@RequestBody RequestEntity requestEntity){
		ResponseEntity responseEntity = new ResponseEntity();
		try {
			TestApplication testApplication = requestEntity.getTestApplication();
			service.dealwithTestApplication(testApplication);
			Map<String, Object> data = new HashMap<String, Object>();
			
			data.put("record", testApplication);
			responseEntity.setData(data);
		} catch (Exception e) {
	...
	public @ResponseBody ResponseEntity add(@RequestBody RequestEntity requestEntity){
		ResponseEntity responseEntity = new ResponseEntity();
		try {
			service.add(requestEntity.getTestApplication());
			Map<String, Object> data = new HashMap<String, Object>();
			
			data.put("record", requestEntity.getTestApplication());
			responseEntity.setData(data);
		} catch (Exception e) {
			responseEntity.setSuccess(CommonConstant.RESPONSE_FAIL);
			responseEntity.setErrorMsg(e.getMessage());
	...
	public @ResponseBody ResponseEntity update(@RequestBody RequestEntity requestEntity){
		ResponseEntity responseEntity = new ResponseEntity();
		try {
			service.update(requestEntity.getTestApplication());
		} catch (Exception e) {
			responseEntity.setSuccess(CommonConstant.RESPONSE_FAIL);
			responseEntity.setErrorMsg(e.getMessage());
		}
