...
public class AnswerRecordManageController {
	
	@Autowired
	AnswerRecordService service;
	...
	public @ResponseBody ResponseEntity findAll(){
		...
		List<AnswerRecord> list = service.list();
		data.put("list", list);
	...
	public @ResponseBody ResponseEntity findByPage(@RequestBody RequestEntity requestEntity){
		...
		List<AnswerRecord> list = service.findListByCondition(requestEntity.getAnswerRecord());
//		List<OrderInfoExtends> extendsList = service.packageOrderInfoExtendsListFrom(list);
	...
	public @ResponseBody ResponseEntity findById(@RequestBody RequestEntity requestEntity){
		...
		AnswerRecord record = service.get(requestEntity.getId());
		AnswerRecordExtends answerRecordExtends = service.packExtendsFrom(record);
		data.put("record", answerRecordExtends);
		responseEntity.setData(data);
		return responseEntity;
	}
	...
	public @ResponseBody ResponseEntity add(@RequestBody RequestEntity requestEntity){
		ResponseEntity responseEntity = new ResponseEntity();
		try {
			service.addAnswerRecord(requestEntity.getAnswerRecord());
			Map<String, Object> data = new HashMap<String, Object>();
			
			data.put("record", requestEntity.getAnswerRecord());
			responseEntity.setData(data);
		} catch (Exception e) {
	...
	public @ResponseBody ResponseEntity update(@RequestBody RequestEntity requestEntity){
		ResponseEntity responseEntity = new ResponseEntity();
		try {
			service.update(requestEntity.getAnswerRecord());
		} catch (Exception e) {
			responseEntity.setSuccess(CommonConstant.RESPONSE_FAIL);
			responseEntity.setErrorMsg(e.getMessage());
		}
